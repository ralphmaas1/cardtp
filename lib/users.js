...
export async function getUserById(userId) {
  ...
  try {
    // Vereenvoudigde query om problemen met foreign keys te vermijden
    ...
    // Vereenvoudigde gebruikersgegevens teruggeven
    return {
      id: user.id,
      name: user.name,
      email: "gebruiker@voorbeeld.com", // Placeholder omdat we geen auth relatie hebben
      avatarUrl: user.avatar_url,
      bio: "",
      website: "",
      phone: "",
      role: "user", // Standaardrol omdat we geen roles relatie hebben
      status: "active", // Standaardstatus
      verified: false,
      lastActive: null,
      createdAt: user.created_at,
      addresses: [],
